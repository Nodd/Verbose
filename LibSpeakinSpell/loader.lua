...
-- Verbose.lua is loaded last, and runs its OnLoad function after every file has loaded
...
--Verbose = LibStub("AceAddon-3.0"):NewAddon("Verbose", "AceConsole-3.0", "AceEvent-3.0", "AceComm-3.0", "AceSerializer-3.0")

-------------------------------------------------------------------------------
-- CURRENT VERSION
-------------------------------------------------------------------------------

-- Verbose version specified in the TOC file uses the format:
-- WoW version + SS Release version, ex. "6.0.3.xx"
Verbose.CURRENT_VERSION = GetAddOnMetadata("Verbose", "version")

-- so the target WoW version is a substring of that...
-- (used in "/ss ad" messages)
Verbose.WOWVERSION = string.sub(Verbose.CURRENT_VERSION, 1, 5) -- ex. "6.0.3"
...
Verbose.DEVELOPER_MODE = false


-------------------------------------------------------------------------------
-- CONSTANTS
-------------------------------------------------------------------------------
Verbose.URL = "Search www.curse.com for \"Verbose\""
...
Verbose.Colors = {
	-- TODOFUTURE: (maybe never?) make this color configurable, it's used for the Verbose system message color
...
Verbose.DEFAULTS = {
	...
	VerboseSavedData = {
		...
		--Version = Verbose.CURRENT_VERSION, -- leave undefined until validated at least once at creation time
		...
		Colors = {
			...
			-- will be copied into Verbose.Colors as |c codes
			Channels = {
	...
	RuntimeData = {
		...
		Networking = {
			NewestVersion = Verbose.CURRENT_VERSION,
		},
		SetupGuides = {
		},
		OptionsGUIStates = {
...
VerboseSavedData = {} -- character specific data
VerboseSavedDataForAll = {} -- account-wide data
