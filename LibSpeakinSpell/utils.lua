...
Verbose:PrintLoading("utils.lua")
...
function Verbose:RemoveEmptyTables( t )
...
function Verbose:AddTable(Dest, Source)
...
function Verbose:StringArray_Compress( List )
...
function Verbose:IsTableEmpty( t )
	if not t then
		return true
	end
...
function Verbose:GetPlayerFullTitleName()
...
function Verbose:PlayerNameNoRealm( NameWithRealm )
...
function Verbose:NameIsMe( InputName )
	--NOTE: myrealm result from UnitName("player") is always nil
...
function Verbose:GetDefaultTarget(ShowDebugMsg)
...
function Verbose:FormatDisplayName( de, DisplayNameFormat )
...
function Verbose:NameToUnitID( name )
...
function Verbose:StartsWith(s, prefix)
	start = string.sub(s, 1, string.len(prefix))
	return (start == prefix)
end


function Verbose:EndsWith(s, suffix)
	strend = string.sub(s, -1 * string.len(suffix))
	return (strend == suffix)
end


function Verbose:ContainsWholeWord(s, word)
...
function Verbose:IsInWorldPVPBattle()
...
function Verbose:GetRandomTableEntry( t, last )
...
function Verbose:CheckForInstance()
	local isInstance, instanceType = IsInInstance()
	return isInstance
end

function Verbose:GetScenarioKey()
...
function Verbose:GetRacialLanguage()
...
function Verbose:GetClassLanguage()
...
function Verbose:ValidateObject( obj, DefaultObject )
...
function Verbose:GetAddonMemoryUsedString()
	-- start
	local total = 0
...
function Verbose:LoadChatColorCodes()
	...
	for channel,_ in pairs( Verbose.Colors.Channels ) do
		local info = ChatTypeInfo[channel]
		if info then
			if nil == info.a then
				info.a = 1
			end
			Verbose.Colors.Channels[channel] = string.format( "|c%02x%02x%02x%02x", 255*info.a, 255*info.r, 255*info.g, 255*info.b )
		end
	end

	-- create colors code strings for the Verbose-created special channels
	if	VerboseSavedData and
		VerboseSavedData.Colors and
		VerboseSavedData.Colors.Channels then
		for channel,info in pairs( VerboseSavedData.Colors.Channels ) do
			if nil == info.a then
				info.a = 1
			end
			Verbose.Colors.Channels[channel] = string.format( "|c%02x%02x%02x%02x", 255*info.a, 255*info.r, 255*info.g, 255*info.b )
...
function Verbose:StringColorCodeToTable(cc)
...
function Verbose:DamageSchoolCodeNumberToString(number)
	local Schools = {
	...
	if Schools[number] then
		...
		return L["Unknown Damage Type"] --REVIEW: for consistency with the rest of Verbose, should this return nil? other <substitutions> fail to substitute, rather than say "No Target"
	end
end
...
function Verbose:GetActiveCompanion(CritterOrMount)
	return nil, nil  --not in Classic
	--[[
...
--[[
function Verbose:DetermineMountClass( tooltip )
	...
	-- code style and actions taken have been changed for Verbose purposes
...
---
	...
	if not VerboseTempEditBox then
		self:DebugMsg(funcname, "creating VerboseTempEditBox")
		VerboseTempEditBox = CreateFrame("EditBox", "VerboseTempEditBox")
		--ZOMG HAX
		...
		--and took a WILD guess that it was a field of VerboseTempEditBox
		--one nil field error after the next, I experimentally found I could hack around it by adding these stubs
		VerboseTempEditBox.header = {}
		VerboseTempEditBox.header.Hide = function() end
		VerboseTempEditBox.focusLeft = {}
		VerboseTempEditBox.focusLeft.Hide = function() end
		VerboseTempEditBox.focusRight = {}
		VerboseTempEditBox.focusRight.Hide = function() end
		VerboseTempEditBox.focusMid = {} --focusMid?! Blizz, you are just mocking me now WTF are these things?!
		VerboseTempEditBox.focusMid.Hide = function() end
		VerboseTempEditBox.button = {} --TODOFUTURE: this isn't it, it's a field of some other table
		--Hide was already here before 3.3.5, since the beginning
		VerboseTempEditBox:Hide()
	end

	self:DebugMsg(funcname,"executing slash command: " .. tostring(text) )
	VerboseTempEditBox:SetText(text)
	ChatEdit_SendText(VerboseTempEditBox) -- throws a lua error, as-is, without more attached frame objects in the table
...
see frames.xml
	<EditBox name="VerboseEditBoxTemplate" inherits="ChatFrameEditBoxTemplate" virtual="true">
		<Scripts>
			<OnLoad>
...
function Verbose:RunSlashCommand(text)
	...
	-- wait to create the VerboseChatEditBox until the first time we need it
	if not Verbose.VerboseChatEditBox then
		--Verbose.VerboseChatEditBox = CreateFrame("EditBox", "VerboseChatEditBox", UIParent, "VerboseEditBoxTemplate")
		Verbose.VerboseChatEditBox = ChatFrame2EditBox --HACK!
	end

	-- OK to send the command
	self:DebugMsg(funcname,"passing to VerboseChatEditBox: " .. tostring(text) )
	Verbose.VerboseChatEditBox:SetText(text)
	ChatEdit_SendText(Verbose.VerboseChatEditBox)
...
--function Verbose:RunSlashCommand(text)
	...
	--		"/c 5 test" works
	--		"/c mychannel test" fails silently within Verbose
...
--Ris: This is what I've always wanted to do in Verbose to begin with
