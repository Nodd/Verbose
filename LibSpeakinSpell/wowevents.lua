...
--local Verbose = LibStub("AceAddon-3.0"):GetAddon("Verbose-0.1")
local L = LibStub("AceLocale-3.0"):GetLocale("Verbose", false)

Verbose:PrintLoading("wowevents.lua")
...
function Verbose:RegisterAllEvents()
...
function Verbose:VARIABLES_LOADED()
	local funcname = "VARIABLES_LOADED"
	self:DebugMsg(funcname, "entry")

	--self:OnVariablesLoaded() -- redirected to Verbose.lua near OnLoad and OnInitialize
end

--]]


function Verbose:ADDON_LOADED(_,name)
...
function Verbose:SPELL_AURA_APPLIED(timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
	local funcname = "SPELL_AURA_APPLIED"
	-- we're only concerned with buffs we receive
	if not Verbose:NameIsMe(dstName) then
...
--	self:DebugMsgDumpString("spellName",spellName)
	...
	if not Verbose:NameIsMe(srcName) then
...
function Verbose:SPELL_AURA_REMOVED(timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
...
function Verbose:SPELL_DAMAGE(timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
	-- extra parameters provided with "SPELL_" prefix
	local spellId, spellName, spellSchool = select(1, ...)
...
function Verbose:SWING_DAMAGE(timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
...
function Verbose:RANGE_DAMAGE(timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
...
function Verbose:ENVIRONMENTAL_DAMAGE(timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
	-- extra parameters provided with "RANGE_" prefix
	local environmentalType = select(1, ...)
	-- extra parameters provided with "_DAMAGE" suffix
	local amount, overkill, school, resisted, blocked, absorbed, critical, glancing, crushing = select(2, ...)
	-- transfer to shared function
	self:DamageEvent(eventtype, spellId, spellName, srcName, dstName, amount, overkill, school, critical)
end

--[[ Not ready yet
function Verbose:SPELL_MISSED(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
...
function Verbose:DamageEvent(eventtype, spellId, spellName, srcName, dstName, amount, overkill, school, critical)

	local funcname = "DamageEvent"

	if Verbose:NameIsMe(dstName) then
		local DetectedEventStub = {
			type = "COMBAT",
		...
		--DetectedEventStub.displaylink = self:FormatSubs( "<eventtypeprefix><eventname>: <spelllink>", DetectedEventStub)  -- TEST
...
Verbose.CombatLogEvents = {
	["SPELL_AURA_APPLIED"] = function(  timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
		-- buff gains
		Verbose:SPELL_AURA_APPLIED(timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
	end,
	...
	["SPELL_AURA_REMOVED"] = function(  timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
		...
		Verbose:SPELL_AURA_REMOVED(timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
	end,
	["SPELL_DAMAGE"] = function(  timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
		...
		Verbose:SPELL_DAMAGE( timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
	end,
	["SWING_DAMAGE"] = function(  timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
		...
		Verbose:SWING_DAMAGE( timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
	end,
	["RANGE_DAMAGE"] = function(  timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
		...
		Verbose:RANGE_DAMAGE( timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
	end,
	["ENVIRONMENTAL_DAMAGE"] = function(  timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
		-- white hits with a ranged attack, such as a hunter's bow, or a mage's wand, etc.
		Verbose:ENVIRONMENTAL_DAMAGE( timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
	end,
...
--		Verbose:SPELL_MISSED( timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
--	end,
}

function Verbose:COMBAT_LOG_EVENT_UNFILTERED()
	...
	self:Print(eventtype)
	local func = Verbose.CombatLogEvents[eventtype]
	if func then
...
function Verbose:UNIT_THREAT_LIST_UPDATE(_,mob)
...
Added to Verbose in SS v5.0.4.02
...
For Verbose, we really only care about the cases where You, the player,
...
--]]
function Verbose:COMPANION_UPDATE(_, CritterOrMount)
...
function Verbose:UNIT_SPELLCAST_SENT(event, caster, target, castid, spellid)
...
function Verbose:UNIT_SPELLCAST_START(event, caster, castid, spellid)
...
function Verbose:UNIT_SPELLCAST_INTERRUPTED(event, caster, castid, spellid)
...
function Verbose:UNIT_SPELLCAST_FAILED(event, caster, castid, spellid)
	local spellname = GetSpellInfo(spellid)
	self:OnSpellcastEvent(event, caster, spellname, spellid)
	PendingSpellcastsSent[spellname] = nil
end

function Verbose:UNIT_SPELLCAST_CHANNEL_START(event, caster, castid, spellid)
	local spellname = GetSpellInfo(spellid)
	self:OnSpellcastEvent(event, caster, spellname, spellid)
end

function Verbose:UNIT_SPELLCAST_CHANNEL_STOP(event, caster, castid, spellid)
...
function Verbose:UNIT_SPELLCAST_STOP(event, caster, castid, spellid)
	local spellname = GetSpellInfo(spellid)
	self:OnSpellcastEvent(event, caster, spellname, spellid)
	PendingSpellcastsSent[spellname] = nil
end

function Verbose:UNIT_SPELLCAST_SUCCEEDED(event, caster, castid, spellid)
	local spellname = GetSpellInfo(spellid)
	self:OnSpellcastEvent(event, caster, spellname, spellid)
...
function Verbose:OnSpellcastEvent(event, caster, spellname, spellid)
--function Verbose:OnSpellcastEvent(event, caster, spellname, spellrank, spellid)
...
function Verbose:PLAYER_DEAD(event)
...
function Verbose:PLAYER_ALIVE(event)
...
function Verbose:PLAYER_UNGHOST(event)
	self.RuntimeData.dead = false
end
...
function Verbose:PLAYER_REGEN_DISABLED()
...
function Verbose:PLAYER_REGEN_ENABLED()
...
function Verbose:OnZoneChange(minoronly)
...
--[[
function Verbose:MINIMAP_ZONE_CHANGED()
...
function Verbose:ZONE_CHANGED_INDOORS()
...
function Verbose:ZONE_CHANGED_NEW_AREA()
...
--]]
function Verbose:CHAT_MSG_WHISPER(event, msg, author, language, status, id, ...)
	-- Chat Event: Whispered While In-Combat
	if	self.RuntimeData.InCombat			and
		type(author) == "string"			and
		not Verbose:NameIsMe(author)	then
...
function Verbose:CHAT_MSG_BN_WHISPER(event, msg, author, language, status, id, ...)
...
--]]
function Verbose:CHAT_MSG_GUILD(event, message, sender, ...)
...
function Verbose:CHAT_MSG_PARTY(event, message, sender, ...)
...
[[A party member said ding]]
			...
			caster = self:PlayerNameNoRealm(sender),
			--target = Verbose:GetGuildName(), -- assume default target logic
		}
		self:OnSpeechEvent( DetectedEventStub )
	end
end
...
function Verbose:AUTOFOLLOW_BEGIN(event, unit)
...
function Verbose:AUTOFOLLOW_END(event, ...)
...
function Verbose:ACHIEVEMENT_EARNED(event, AchievementID)
...
--]]
function Verbose:CHAT_MSG_ACHIEVEMENT(event, ChatMessage, EarnedBy, ...)
	-- don't congratulate myself (we DO get this message for our own achievements)
	if Verbose:NameIsMe(EarnedBy) then
		return
	end
...
--]]
function Verbose:CHAT_MSG_GUILD_ACHIEVEMENT(event, ChatMessage, EarnedBy, ...)
	if Verbose:NameIsMe(EarnedBy) then -- don't congratulate myself (we DO get this message for our own achievements)
		return
	end
	local ndx = string.find(ChatMessage, "|") -- find the clickable link in the achievement message
	local achievement = string.sub(ChatMessage, ndx, string.len(ChatMessage))
...
function Verbose:BARBER_SHOP_OPEN()
	local DetectedEventStub = {
		type = "NPC",
...
function Verbose:BARBER_SHOP_CLOSE()
	local DetectedEventStub = {
		type = "NPC",
...
function Verbose:MERCHANT_SHOW()
	local DetectedEventStub = {
		type = "NPC",
...
function Verbose:QUEST_GREETING()
	local DetectedEventStub = {
		type = "NPC",
...
function Verbose:TAXIMAP_OPENED()
	local DetectedEventStub = {
		type = "NPC",
...
function Verbose:TRAINER_SHOW() -- fired for any class and profession trainers
	local DetectedEventStub = {
		type = "NPC",
...
-- CONFIRM_SUMMON and CANCEL_SUMMON are not triggered when i attempt to summon myself without assistance
 ...
 -- I received a summon?
function Verbose:CONFIRM_SUMMON(...)
...
function Verbose:CANCEL_SUMMON(...)
...
--]]
function Verbose:PLAYER_LEVEL_UP(event, ...)
	local DetectedEventStub = {
		type = "EVENT",
...
function Verbose:TRADE_SHOW()
 ...
 function Verbose:UPDATE_MOUSEOVER_UNIT()
